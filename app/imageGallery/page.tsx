...
import config from "@/config/Setup.json"; // 🔹 Import your custom config
...
function ImageGalleryContent() {
  ...
  useEffect(() => {
    if (imageId) {
      retrieveImageAndSearch(imageId);
    }
  }, [imageId]);

  const retrieveImageAndSearch = (id: string) => {
      ...
      <main>
        <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
          ...
          {similarImages.length > 0 && (
            <div>
              <h2 className={`text-xl font-semibold mb-4 ${config.headingColor}`}>Similar Images</h2>
